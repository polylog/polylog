...
|-------------------------------------------------------------------------------
|
| Переменные примитивных типов данных включаются в шаблон только через
| конструкцию print - <?php print $variable; ?>
| переменные-массивы — еще и через функциюю render()
| <?php print render($tabs); ?>
| особый случай массивы меню — они включаются с помощью theme()
| <?php print theme('links__system_main_menu', array('links' => $main_menu, 'attributes' => array('class' => 'mm'))); ?>
|
...
| - $page['frontpage_content'] - Содержание первой страницы. TODO: Проверить —
...
| - $base_path - Путь к каталогу, в котором установлен В. По умолчанию — /. Для
|   ссылки на первую страницу рекомендуется использовать $front_page —
|   в зависимости от языка и настроек будет добавлен субдомен или каталог.
| - $breadcrumb - «Хлебные крошки» для текущей страницы.
| - $directory - Директория, в которой содержится шаблон данного уровня. В случае
|   с page.tpl.php - modules/system или themes/new_site2013.
| - $feed_icons — Строка пиктограмм-ссылок для всех RSS-лент доступных с текущей
|   страницы.
| - $front_page — URL первой страницы. По умолчанию — /. В зависимости от языка
|   и настроек будет добавлен субдомен или каталог.
| - $is_admin - TRUE, если пользователь имеет доступ к административному разделу.
| - $logged_in - TRUE, если сайт просматривает пользователь, который зарегистрирован
|   и авторизован.
...
|       …
|   <?php endif; ?>
| - $node - Узел, основное содержание страницы, если в системном URL ID узла
|   находится во второй позиции. Например, node/1234 и node/1234/revisions,
|   но не comment/reply/1234.
| - $secondary_menu - Массив ссылок вспомогательного меню. Пуст, если меню
|   не настраивалось.
| - $secondary_menu_heading - Заголовок вспомогательного меню.
| - $tabs - Массив вкладок со ссылками на вложенные структуры и различные
|   состояния текущей страницы — например, «Смотреть», «Правка».
